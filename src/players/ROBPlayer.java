...
public class ROBPlayer extends Player {
     ...
     */
    public ROBPlayer(long seed, int id) {
        super(seed, id);
        reset(seed, id);
    }
    ...
    @Override
    public Player copy() {
        ROBPlayer player = new ROBPlayer(seed,playerID);
    ...
    public Types.ACTIONS act(GameState gs) {
        ...
        ArrayList<Types.ACTIONS> actionsList = new ArrayList<Types.ACTIONS>();
        ...
        if(!unsafeDirections.isEmpty()){
            ...
            if(!directions.isEmpty()) {
                actionsList.add(directionToAction(directions.get(random.nextInt(directions.size()))));
            }
            else {
                actionsList.add(Types.ACTIONS.ACTION_STOP);
            }
        }
        ...
        if(isAdjacentEnemy(items, dist, enemies) && maybeBomb(ammo, blastStrength, items, dist, myPosition)){
            actionsList.add(Types.ACTIONS.ACTION_BOMB);
        }
        ...
        for (GameObject en: enemies){
            ...
            while (dist_it.hasNext()){
                ...
                if (entry.getKey().equals(en.getPosition()) && entry.getValue() == 3){
                    ...
                    actionsList.add(directionToAction(getDirection(myPosition, next_node)));
        ...
        if (distance <= 2){
            ...
            if (myPosition.equals(previousNode)){
                actionsList.add(directionToAction(getDirection(myPosition, previousNode)));
            }
                ...
                for (Vector2d coords:entry.getValue()){
                    ...
                    if (dist.get(coords) <= 2){
                        ...
                        if (direction != null){
                            ...
                            if (dirArray.size() > 0){
                                actionsList.add(directionToAction(dirArray.get(0)));
                            }
                        }
        ...
        if (validDirections.size() > 0){
            ...
            actionsList.add(directionToAction(validDirections.get(actionIdx)));
        }
        ...
        for (Types.ACTIONS act : actionsList) {
            ...
            double Q = Utils.noise(valState, this.epsilon, this.random.nextDouble()) + (actionsList.size() - i) * 0.25; // CHANGED
